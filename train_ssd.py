...
if __name__ == '__main__':
    ...
    for epoch in range(last_epoch + 1, args.num_epochs):
        ...
        train(train_loader,
              ...
              epoch=epoch)
        t_elapsed_time = time.time() - t_start
        train_time.append(t_elapsed_time)

        # do validation
        v_start = time.time()
        val_loss, val_regression_loss, val_classification_loss = test(val_loader, pruned_net, criterion, DEVICE)
        v_elapsed_time = time.time() - v_start
        val_time.append(v_elapsed_time)

        model_loss.append([val_loss, val_regression_loss, val_classification_loss])
